...
void main() {
	...
	if (success) {
		...
		for(i32 i = 1; i < light_count; i++ ) {
			f32 x = f32(random(push_constant.mersenne_id)) / f32(~0u) * 2 - 1;
			f32 y = f32(random(push_constant.mersenne_id)) / f32(~0u);
			f32 z = f32(random(push_constant.mersenne_id)) / f32(~0u) * 2 - 1;
		
					
			lights[i].position = vec3(x * 10000 + 10000, 10000 * y + 10000, z * 10000 + 10000); 
