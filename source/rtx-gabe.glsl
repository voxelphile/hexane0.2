...
#define TRACE_STATE_BLOCK 1
#define TRACE_STATE_LIGHT_SETUP 2
#define TRACE_STATE_LIGHT 3
...
void reset_trace_state(in out TraceState trace_state) {
	...
	trace_state.initial_ray.result_i = uvec2(0);
...
;
	...
	if (success) {
		...
		if(is_solid(u16(trace_state.ray_state.ray.medium))) {
		Ray ray;
		ray.direction = ray_hit.ray.direction;
		ray.region_data = region.blocks;
		ray.result_i = trace_state.ray_state.ray.result_i;
		ray.true_dir = trace_state.ray_state.ray.true_dir;
		ray.max_lod = 0;
		ray.origin = fract(ray_hit.ray.origin - ray_hit.normal * EPSILON) * BLOCK_DETAIL;
		ray.medium = u16(1);
		ray.max_distance = ray_hit.dist * BLOCK_DETAIL;
		ray.minimum = vec3(0);
		ray.maximum = vec3(BLOCK_DETAIL);
		ray_cast_start(ray, trace_state.ray_state);
		trace_state.rays++;


		trace_state.id = TRACE_STATE_BLOCK;
		return false;
		}
	...
	if(trace_state.id == TRACE_STATE_BLOCK && !ray_cast_drive(trace_state.ray_state)) {	
	RayHit ray_hit;
    	
	bool success = ray_cast_complete(trace_state.ray_state, ray_hit);
	
	if(trace_state.ray_state.id == RAY_STATE_MAX_DIST_REACHED) {
		trace_state.color.xyz = vec3(0, 0, 1);
		return true;
	}
	
	if(trace_state.ray_state.id == RAY_STATE_OUT_OF_BOUNDS) {
		trace_state.ray_state.id = RAY_STATE_INITIAL;
		trace_state.ray_state.map_pos = clamp(mod(trace_state.ray_state.map_pos - 1 + BLOCK_DETAIL, BLOCK_DETAIL), EPSILON, BLOCK_DETAIL - EPSILON);
		return true;
	}
	...
	if (success) {
		...
		trace_state.color.xyz *= 0.1 + 0.9 * mix(mix(ambient.z, ambient.w, ray_hit.uv.x), mix(ambient.y, ambient.x, ray_hit.uv.x), ray_hit.uv.y);
	...
	while(count < STALL_LIMIT) {
		...
		if(subgroupAny(!trace_state.currently_tracing)) {
			if(!trace_state.currently_tracing) {
				...
				i32 ray_cache_index = CACHE_SIZE - atomicAdd(ray_batch.setup_cache.size, -1);
